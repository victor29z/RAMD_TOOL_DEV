...
static u8 TEST_SPI_DATA_SIZE_SELECT[] = {TEST_SPI_1BYTE_DATA, TEST_SPI_2BYTE_DATA, TEST_SPI_4BYTE_DATA};
static u8 TEST_SPI_ADDR_SIZE_SELECT[] = {TEST_SPI_2BYTE_ADDR, TEST_SPI_3BYTE_ADDR};
 ...
 * ----------------------------------------------------------------------------
 */
int BSP_PDI_FunRead(u32 addr, u8 *pbuf, u32 ByteSize)
{
	
	HW_FunRead((MEM_ADDR*)pbuf, (UINT16)addr, (UINT16)ByteSize);
	
	return 0;
} /* End of BSP_PDI_FunRead() */

/*
 * ----------------------------------------------------------------------------
 * Function Name: BSP_PDI_FunWrite()
 * Purpose:
 * Params:
 * Returns:
 * Note:
 * ----------------------------------------------------------------------------
 */
int BSP_PDI_FunWrite(u32 addr, u8 *pbuf, u32 ByteLen)
{

	HW_FunWrite((MEM_ADDR*)pbuf, (UINT16)addr, (UINT16)ByteLen);

	return 0;
} /* End of BSP_PDI_FunWrite() */
 ...
 * ----------------------------------------------------------------------------
 */
int BSP_PDI_EscRead(u32 addr, u8 *pbuf, u32 ByteSize)
{
 ...
 * ----------------------------------------------------------------------------
 */
int BSP_PDI_EscWrite(u32 addr, u8 *pbuf, u32 ByteLen)
{
...
s16 TEST_PdiMemoryTest(TEST_CONTROL *pCtrl) 
{
		...
		case TEST_IDLE:
			...
			memset((u8*)&pCtrl->Temp, 0, sizeof(pCtrl->Temp));
			memset((u8*)&pCtrl->Record, 0, sizeof(pCtrl->Record));			
		...
		case TEST_RUN:
			...
			/* Write to target */		
			if (pParameter->ChipSelectMode)
			{
				BSP_PDI_EscWrite(pTemp->AddrOffset, pTemp->TxBuf, pTemp->TxLen);
			}
			else
			{
				BSP_PDI_FunWrite(pTemp->AddrOffset, pTemp->TxBuf, pTemp->TxLen);					
			}
#if 0
			MISC_GET_HOST_INTF_STATUS(&pTemp->HostInterfaceErrorStatus);
			if (pTemp->HostInterfaceErrorStatus)
			{
				DBG_TriggerOutput(1);
				DBG_TriggerOutput(0);
				pRecord->ErrorCnt++;
				pCtrl->State = TEST_ERROR;
				ESC_DEBUG("Host interface status error(0x%x) in write operation!\r\n", pTemp->HostInterfaceErrorStatus);
				break;
			}
#endif
			...
			{
				BSP_PDI_FunRead(pTemp->AddrOffset, pTemp->RxBuf, pTemp->RxLen);
			}
#if 0
			MISC_GET_HOST_INTF_STATUS(&pTemp->HostInterfaceErrorStatus);					
			if (pTemp->HostInterfaceErrorStatus)
			{
				...
				pRecord->ErrorCnt++;
				pCtrl->State = TEST_ERROR;				
				ESC_DEBUG("Host interface status error(0x%x) in write operation2!\r\n", pTemp->HostInterfaceErrorStatus);
				break;
			}
#endif
			/* Check result */
			if (memcmp(pTemp->TxBuf, pTemp->RxBuf, pTemp->TxLen))
			{
				...
				pRecord->ErrorCnt++;
				pCtrl->State = TEST_ERROR;
				ESC_DEBUG("Pattern comparing error!\r\n");
				break;				
			}
